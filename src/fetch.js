import { display_GenreButtons, display_Movies, display_Spinner } from './render.js';
...
export async function fetch_Movies() {
    ...
    try {
        const response = await fetch(`${TMDB_URL}/movie/now_playing?language=ko-KR&page=1`, options);
...
async function fetchMovieTrailers(movieId) {
    try {
        const response = await fetch(`${TMDB_URL}/movie/${movieId}/videos?language=en-US`, options);
        const data = await response.json();
...
export async function fetch_SearchMovies(keyword) {
    try {
        const response = await fetch(
            `${TMDB_URL}/search/movie?query=${encodeURIComponent(keyword)}&language=ko-KR&page=1&include_adult=false`,
            options
        );
        const data = await response.json();
...
export async function fetch_SearchByGenre(genreId) {
    display_Spinner(true);
    await fetch(`${TMDB_URL}/discover/movie?with_genres=${genreId}&language=ko-KR&page=1`, options)
        .then((response) => response.json())
        .then((data) => {
