import { display_JenreButtons, display_Movies, display_Spinner } from './render.js';
...
export async function fetch_Movies() {
    ...
    try {
        const response = await fetch(`${TBDB_URL}/movie/now_playing?language=ko-KR&page=1`, options);
        const data = await response.json();
        display_Spinner(false);
        display_Movies(data.results, document.getElementById('movieList')); // 영화 목록을 화면에 표시
...
async function fetchMovieTrailers(movieId) {
    ...
    try {
        const response = await fetch(`${TBDB_URL}/movie/${movieId}/videos?language=en-US`, trailerOptions);
        const data = await response.json();
...
async function updateSliderWithMovies(movies) {
    ...
    for (const movie of movies) {
        ...
        const trailerEmbedUrl = trailerKey ? `https://www.youtube.com/embed/${trailerKey}?autoplay=0&mute=1` : '';
...
export async function fetch_SearchMovies(keyword) {
    try {
        const response = await fetch(
            `${TBDB_URL}/search/movie?query=${encodeURIComponent(keyword)}&language=ko-KR&page=1&include_adult=false`,
            options
        );
        const data = await response.json();
...
export async function fetch_SearchByJenre(genreId) {
    display_Spinner(true);
    await fetch(`${TBDB_URL}/discover/movie?with_genres=${genreId}&language=ko-KR&page=1`, options)
        .then((response) => response.json())
        .then((data) => {
